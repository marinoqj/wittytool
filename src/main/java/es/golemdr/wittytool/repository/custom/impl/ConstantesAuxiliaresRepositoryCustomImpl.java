...
public class ConstantesAuxiliaresRepositoryCustomImpl implements ConstantesAuxiliaresRepositoryCustom{

    @PersistenceContext
    private EntityManager em;

	@Transactional
	public List<ConstanteAuxiliar> findConstantesAuxiliares(ConstanteAuxiliar filtro, PaginacionBean paginacion) {

		List<ConstanteAuxiliar> resultado = null;

		CriteriaBuilder criteriaBuilder = em.getCriteriaBuilder();
		CriteriaQuery<ConstanteAuxiliar> query = criteriaBuilder.createQuery(ConstanteAuxiliar.class);
		Root<ConstanteAuxiliar> constanteauxiliar = query.from(ConstanteAuxiliar.class);
		...
		query.select(constanteauxiliar).where(criteriaBuilder.and(predicates.toArray(new Predicate[predicates.size()])));
