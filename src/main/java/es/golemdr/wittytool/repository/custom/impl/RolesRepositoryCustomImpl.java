...
public class RolesRepositoryCustomImpl implements RolesRepositoryCustom{
	...
	@Transactional
	public List<Rol> findRoles(Rol filtro, PaginacionBean paginacion) {
		...
		query.select(rol).where(criteriaBuilder.and(predicates.toArray(new Predicate[predicates.size()])));
