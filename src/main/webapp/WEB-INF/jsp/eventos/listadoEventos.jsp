...
function borrarEvento(idEvento){
	 	...
	 	document.formulario.idEvento.value = idEvento;
	 	document.formulario.action="borrarEvento.do";
...
function editarEvento(idEvento){
 	...
 	document.formulario.idEvento.value = idEvento;
 	document.formulario.action="editarEvento.do";
 	document.formulario.submit();


}

function mostarConfirmBorrarEvento(idEvento) {
    var message = '<spring:message code="confirmacion.borrar.evento"/>';

    $("#aceptarBorrar").attr("href", "javascript:borrarEvento(" + idEvento + ");");
				...
				<nav aria-label="breadcrumb">
					<ol class="breadcrumb">
						<li class="breadcrumb-item active"><i
							...
							class="fas fa-users fa-lg mr-2"></i>Listado de eventos</li>
					</ol>
				</nav>
			</div>
		</div>
...
<div class="row">
	...
	<c:if test="${hayFiltro eq false}">
		<mistags:paginacion accion="listadoEventos" />
	</c:if>

	<c:if test="${hayFiltro eq true}">
		<mistags:paginacion accion="listadoEventosFiltrado" />
	</c:if>
</div>
	...
	<table class="table table-hover">
		<thead class="blue lighten-4">
			<tr class="bg-light">
					...
					<th scope="col"><spring:message code="label.palabrasClave"/></th>
					
					<th scope="col"><spring:message code="label.observaciones"/></th>
		...
		<c:forEach items="${eventos}" var="evento">
			<tr>
				...
				<td>${evento.palabrasClave}</td>
			
				<td>${evento.observaciones}</td>
			
				<td>${evento.fecha}</td>
			
				<td>${evento.idProyecto}</td>
					...
					<div>
							...
							<ul class="dropdown-menu dropdown-primary">
								<li class="nav-item dropdown"><a class="dropdown-item"
									href="javascript:editarEvento('${evento.idEvento}')"><spring:message code="label.editar"/></a></li>
								<li class="nav-item dropdown"><a class="dropdown-item"
									href="javascript:mostarConfirmBorrarEvento('${evento.idEvento}')"><spring:message code="label.borrar"/></a></li>
							</ul>
						</div>
				</td>
			</tr>
		...
		<c:if  test="${empty eventos}">
			...
			<br>
			<br>
			<div class="text-center">No hay eventos que mostrar...</div>
		</c:if>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header border-bottom-0" style="background-color: #e9ecef; color: #6c757d">
        <span class="modal-title" id="exampleModalLabel"><i class="fas fa-plus-circle fa-lg pr-1"></i><i class="fas fa-cube fa-lg pr-2"></i>Nueva evento</span>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body" style="color: #6c757d">
		<form:form modelAttribute="evento" action="insertarEvento.do" method="post">
					...
					<div class="form-group">
						<label for="palabrasClave"><spring:message code="label.palabrasClave"/></label> <form:input path="palabrasClave" class="form-control"/>
					</div>
					...
					<div class="form-group">
						<label for="observaciones"><spring:message code="label.observaciones"/></label> <form:input path="observaciones" class="form-control"/>
					</div>
...
<div class="modal fade" id="ventanaBuscar" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body" style="color: #6c757d">
		<form:form modelAttribute="evento" action='buscarEventos.do' method="post" id="formularioBuscar">
