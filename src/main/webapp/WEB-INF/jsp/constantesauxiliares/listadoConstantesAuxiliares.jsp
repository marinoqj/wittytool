...
<script>
function borrarConstanteAuxiliar(idConstanteAuxiliar){

	 	document.formulario.idConstanteAuxiliar.value = idConstanteAuxiliar;
	 	document.formulario.action="borrarConstanteAuxiliar.do";
	 	document.formulario.submit();

}
function editarConstanteAuxiliar(idConstanteAuxiliar){

 	document.formulario.idConstanteAuxiliar.value = idConstanteAuxiliar;
 	document.formulario.action="editarConstanteAuxiliar.do";
 	document.formulario.submit();


}

function mostarConfirmBorrarConstanteAuxiliar(idConstanteAuxiliar) {
    var message = '<spring:message code="confirmacion.borrar.constanteauxiliar"/>';

    $("#aceptarBorrar").attr("href", "javascript:borrarConstanteAuxiliar(" + idConstanteAuxiliar + ");");
...
<form name="formulario" method="post">
	<input type="hidden" name="idConstanteAuxiliar"/>
				...
				<nav aria-label="breadcrumb">
					<ol class="breadcrumb">
						<li class="breadcrumb-item active"><i
							...
							class="fas fa-users fa-lg mr-2"></i>Listado de constantesauxiliares</li>
					</ol>
				</nav>
			</div>
		</div>
...
<c:if  test="${!empty constantesauxiliares}">
...
<div class="row">
	...
	<c:if test="${hayFiltro eq false}">
		<mistags:paginacion accion="listadoConstantesAuxiliares" />
	</c:if>

	<c:if test="${hayFiltro eq true}">
		<mistags:paginacion accion="listadoConstantesAuxiliaresFiltrado" />
	</c:if>
</div>
...
<br>
	...
	<table class="table table-hover">
		<thead class="blue lighten-4">
			...
			<tbody>
		<c:forEach items="${constantesauxiliares}" var="constanteauxiliar">
			<tr>
			
				<td>${constanteauxiliar.familia}</td>
			
				<td>${constanteauxiliar.atributo}</td>
			
				<td>${constanteauxiliar.literal}</td>
			
				<td>${constanteauxiliar.valor}</td>
			
				<td>${constanteauxiliar.activa}</td>
					...
					<div>
							...
							<ul class="dropdown-menu dropdown-primary">
								<li class="nav-item dropdown"><a class="dropdown-item"
									href="javascript:editarConstanteAuxiliar('${constanteauxiliar.idConstanteAuxiliar}')"><spring:message code="label.editar"/></a></li>
								<li class="nav-item dropdown"><a class="dropdown-item"
									href="javascript:mostarConfirmBorrarConstanteAuxiliar('${constanteauxiliar.idConstanteAuxiliar}')"><spring:message code="label.borrar"/></a></li>
							</ul>
						</div>
				</td>
			</tr>
		...
		<c:if  test="${empty constantesauxiliares}">
			...
			<br>
			<br>
			<div class="text-center">No hay constantesauxiliares que mostrar...</div>
		</c:if>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header border-bottom-0" style="background-color: #e9ecef; color: #6c757d">
        <span class="modal-title" id="exampleModalLabel"><i class="fas fa-plus-circle fa-lg pr-1"></i><i class="fas fa-cube fa-lg pr-2"></i>Nueva constanteauxiliar</span>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body" style="color: #6c757d">
		<form:form modelAttribute="constanteauxiliar" action="insertarConstanteAuxiliar.do" method="post">
...
<div class="modal fade" id="ventanaBuscar" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body" style="color: #6c757d">
		<form:form modelAttribute="constanteauxiliar" action='buscarConstantesAuxiliares.do' method="post" id="formularioBuscar">
